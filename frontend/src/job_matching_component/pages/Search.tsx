  ...
  const fetchSavedJobs = useCallback(async () => {
    try {
      ...
      if (Array.isArray(saved)) {
        const ids = new Set(saved.map((j: any) => j.id || j.jobId))
        setSavedJobIds(ids)
      }
    } catch (err) {
          ...
          {error ? (
            ...
            <div className="glass-panel p-12 text-center flex flex-col items-center">
              ...
              <p className="text-slate-600 mb-6 max-w-md">
                We couldn't find any jobs matching your current search and filter criteria. Try adjusting your filters or
                search terms.
              </p>
              <button
                onClick={() => {
