...
export function MomentumChart({
  ...
  const getTrendIcon = () => {
    switch (trend) {
      ...
      case 'down':
        return (
          <TrendingUp className="w-5 h-5 text-error transform rotate-180" />
        )
      case 'flat':
  ...
  return (
    <div className="modern-card p-5 flex flex-col h-full">
      <div className="flex items-center justify-between mb-4">
        ...
        <div
          className={`px-2.5 py-1 rounded-full text-xs font-bold border flex items-center gap-1.5 ${getTrendColor()}`}
        >
          {getTrendIcon()}
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="bg-slate-50 rounded-lg p-3 border border-slate-100 text-center">
          <div className="text-2xl font-bold text-slate-700">{totalApps}</div>
        ...
        <div className="bg-primary/5 rounded-lg p-3 border border-primary/10 text-center">
          ...
          <div className="text-xs font-medium text-primary/70 uppercase tracking-wider mt-1">
            Interviews
          </div>
        </div>
        <div className="bg-success/5 rounded-lg p-3 border border-success/10 text-center">
          <div className="text-2xl font-bold text-success">{totalOffers}</div>
          <div className="text-xs font-medium text-success/70 uppercase tracking-wider mt-1">
            Offers
          </div>
        </div>
      </div>
          ...
          return (
            <div key={idx} className="flex-1 flex flex-col items-center group">
              ...
              <div className="w-full max-w-[40px] flex flex-col justify-end h-[120px] rounded-t-sm overflow-hidden bg-slate-50 group-hover:bg-slate-100 transition-colors">
                ...
                {appHeight > 0 && (
                  <div
                    className="w-full bg-slate-300 transition-all duration-500 ease-out"
